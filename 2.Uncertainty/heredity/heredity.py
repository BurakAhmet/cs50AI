...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        if not (mother or father):
            ...
            m = prob_helper(mother, one_gene, two_genes)
            f = prob_helper(father, one_gene, two_genes)

            if genes == 1:
                probability *= (1 - m) * f + (1 - f) * m
            elif genes == 2:
                probability *= m * f
            else:
                probability *= (1 - m) * (1 - f)
...
def prob_helper(parent_name, one_gene, two_genes):
...
def update(probabilities, one_gene, two_genes, have_trait, p):
    ...
    for person in probabilities:
        ...
        gene_copy = 2 if person in two_genes else 1 if person in one_gene else 0
...
def normalize(probabilities):
    ...
    for person in probabilities:
        ...
        for genes in probabilities[person]['gene']:
            probabilities[person]['gene'][genes] /= gene_sum
