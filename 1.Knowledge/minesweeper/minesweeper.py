...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # Set for update AI knowledge base
        neighbours = set()

        # Travel the neighbour cells
        for i in range(max(0, cell[0] - 1), min(cell[0] + 2, self.height)):
            for j in range(max(0, cell[1] - 1), min(cell[1] + 2, self.width)):
                neighbour = (i, j)
                # Skip the chosen cell
                if neighbour == cell:
                    continue
                if neighbour not in self.moves_made and neighbour not in self.mines:
                    neighbours.add(neighbour)
                elif neighbour in self.mines:
                    count -= 1
        # Add new knowledge to AI's knowledge base
        ...
        for sentence in self.knowledge:
            ...
            if known_safes:
                safes.update(known_safes)
            if known_mines:
                mines.update(known_mines)
    ...
    def make_random_move(self):
        ...
        # If no such possible moves return None
